function watch (game, board) {
    ...
    var viewer = new Viewer(board, topside, game.players, mode);
...
DNode().connect(function (server) {
    ...
    server.subscribe(function (em) {
        ...
        em.on('created', function (game) {
            if (!(game.id in thumbs)) {
                ...
                server.watch(function (em) {
                    watchGameEmitter(game.id, em)
                });
            }
        });
    ...
    function watchGameEmitter (gid, em) {
    ...
    server.watchAll(watchGameEmitter);
});
