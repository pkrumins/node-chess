...
function MoveGenerator(board,state,square,kingCheck) {
    ...
    function filterKingInCheck(moves) {
        ...
        moves.forEach(function (move) {
            ...
            var pieces = board.pieces.map(function (p) {
                return Hash(p).extract([
                    'code', 'color', 'x', 'y'
                ]).clone.items
            });
                ...
                for (jj = 0; jj < 8; jj++) {
                    ...
                    if (piece2 && piece2.color != piece.color) { // opponents color
                        ...
                        opponentMoves.forEach(function (opponentMove) {
                            ...
                            // white castling square at F1 is under attack or king under attack
                            if (square.x == 4 && square.y == 0 &&
                                move.x == 6 && move.y == 0)
                            {
                                if ((opponentMove.x == 5 || opponentMove.x == 4) &&
                                    opponentMove.y == 0)
                                {
                                    include = false;
                                    return;
                            ...
                            // white castling square at D1 is under attack or king under attack
                            if (square.x == 4 && square.y == 0 &&
                                move.x == 2 && move.y == 0)
                            {
                                if ((opponentMove.x == 3 || opponentMove.x == 4) &&
                                    opponentMove.y == 0)
                                {
                                    include = false;
                                    return;
                            ...
                            // black castling square at F8 is under attack or king under attack
                            ...
                            // white castling square at D8 is under attack or king under attack
                            if (square.x == 4 && square.y == 7 &&
                                move.x == 2 && move.y == 7)
                            {
                                if ((opponentMove.x == 3 || opponentMove.x == 4) &&
    ...
    function KingMoves() {
        ...
        if (piece.color == 'w') {
            ...
            if (square.x == 4 && square.y == 7) {
                if (square.e.empty() && square.e.e.empty()) {
                    if (square.e.e.e.piece && square.e.e.e.piece.color == 'b' &&
                        square.e.e.e.piece.code == 'r' &&
                ...
                if (square.w.empty() && square.w.w.empty() && square.w.w.w.empty()) {
                    if (square.w.w.w.w.piece && square.w.w.w.w.piece.color == 'b' &&
                        square.w.w.w.w.piece.code == 'r' &&
                        !state.blackKingMoved &&
                        !state.rookA8Moved)
                    {
    ...
    function PawnMoves() {
        ...
        if (piece.color == 'w') {
            ...
            var canCaptureW = square.x > 0 && square.nw.piece && square.nw.piece.color == 'b';
            ...
            var canCaptureW = square.x > 0 && square.sw.piece && square.sw.piece.color == 'w';
