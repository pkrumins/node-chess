...
function MoveGenerator(board,state,square,kingCheck) {
    ...
    if (kingCheck) {
        var okMoves = filterThoseWhereKingIsNotInCheck(possibleMoves);
        return okMoves;
    }
    else {
    ...
    function filterThoseWhereKingIsNotInCheck(moves) {
        var ret = [];

        var king; // find king
        for (var x = 0; x < 8; x++) {
            for (var y = 0; y < 8; y++) {
                var piece2 = board.pieceAt(x,y);
                if (piece2) {
            ...
            for (var ii = 0; ii < 8; ii++) {
                ...
                for (jj = 0; jj < 8; jj++) {
                    ...
                    if (piece2 && piece2.color != piece.color) { // opponents color
                        var tmpOpponentMoves = MoveGenerator(boardCopy,stateCopy,piece2,false);
                        for (iii = 0; iii < tmpOpponentMoves.length; iii++) {
                            var tmpOpponentMove = tmpOpponentMoves[iii];
                            if (tmpOpponentMove.x == king.x && tmpOpponentMove.y == king.y) {
                                include = false;
                                break;
                            }
                        }
