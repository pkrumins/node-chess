...
function Board (useBoard) {
    ...
    function initBoard() {
        var y = 0;
        function Row (Ctor, pieces) {
            var x = 0;
            var row = [];
            for (var i = 0; i < pieces.length; i++) {
                var piece = pieces[i];
                row.push(new Square(x, y, Ctor(piece)));
                x++;
            }
            y++;
            return row;
        ...
        var white_pieces = Row(WP, ['r', 'kn', 'b', 'q', 'k', 'b', 'kn', 'r']);
        var white_pawns = Row(WP, ['p', 'p', 'p', 'p', 'p', 'p', 'p', 'p']);
        var empty_row_2 = Row(NullP, ['', '', '', '', '', '', '', '']);
        var empty_row_3 = Row(NullP, ['', '', '', '', '', '', '', '']);
        var empty_row_4 = Row(NullP, ['', '', '', '', '', '', '', '']);
        var empty_row_5 = Row(NullP, ['', '', '', '', '', '', '', '']);
        var black_pieces = Row(BP, ['r', 'kn', 'b', 'q', 'k', 'b', 'kn', 'r']);
        var black_pawns = Row(BP, ['p', 'p', 'p', 'p', 'p', 'p', 'p', 'p']);
