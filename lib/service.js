...
function Service (client, conn) {
    ...
    self.createGame = function (color, cb) {
        ...
        game.on('move', function (sx, sy, dx, dy, choice) {
            games.emit('moved', id, sx, sy, dx, dy, choice);
        });
    ...
    self.quit = function () {
        ...
        if (user.name) delete users[user.name];
    };
    
    conn.on('end', function () { self.quit() });
}
