...
function Viewer (board, topside, players, mode, moveMakerF) {
            ...
            if (piece.color == 'w' && piece.code == 'p' && srcMove.y == 6 && y == 7) {
                ...
                PromotionDialog(im, area, 'w', function (choice) {
                    ...
                    var sprite = im.image(
                        ...
                        sprites[x][y].attr('x'),
                        sprites[x][y].attr('y'),
                        sprites[x][y].attr('width'),
                        sprites[x][y].attr('height')
                    );
                    sprites[x][y].remove();
                    sprites[x][y] = sprite;
                });
    ...
    board.on('move', function (sx, sy, dx, dy) {
        ...
        var t = transform(dx, dy);
        //For some reason, using transform.piece shifts pieces to the right
        //but you need it for initial placement.
        //Don't ask me what's up. >_<
