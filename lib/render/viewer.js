...
function Viewer (board, topside, players, mode, moveMakerF) {
    ...
    board.on('move', function (sx, sy, dx, dy, choice) {
        ...
        // handle en-passant
        var piece = board.pieceAt(dx,dy);
        if (piece.color == 'w' && piece.code == 'p' && sy == 4 && sx != dx) {
            // white en-passant
            var eliminateX = dx;
            var eliminateY = sy;
            board.at(eliminateX,eliminateY).clear();
            console.log(eliminateX, eliminateY);
            sprites[eliminateX][eliminateY].remove();
        }
        if (piece.color == 'b' && piece.code == 'p' && sy == 3 && sx != dx) {
            // black en-passant
            var eliminateX = dx;
            var eliminateY = sy;
            board.at(eliminateX,eliminateY).clear();
