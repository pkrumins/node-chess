function Viewer (board, topside, players, mode, moveMakerF) {
    ...
    function cellClick(cell,x,y) {
        ...
        highlightOnOff(cell,x,y);
        if (srcMove.x !== undefined) {
    ...
    board.on('move', function (sx, sy, dx, dy) {
        ...
        if (dst) {
            ...
            setTimeout(function () {
                //Where's the canvas?
                //var blam = image('images/violence'+violence[0]+'.svg', dx, dy, 35, 35);
                dst.remove(); // <-- more violently
                //blam.remove();
                //var blam = image('images/violence'+violence[1]+'.svg', dx, dy, 35, 35);
                //All this, with some sensible timing code and an actual canvas. Non-hard-coded sizes might be nice too :(
            }, 500);
        }
        sprites[dx][dy] = src;
        sprites[sx][sy] = null;
